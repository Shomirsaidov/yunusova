<template>
    ...
    <div class="flex flex-col md:w-1/2 w-3/4 my-8">
        <div class="p-4 rounded-xl bg-gray-200 flex items-center rounded-b-none">
            ...
            <span class="font-lg font-semibold">Должники</span>
        </div>
        ...
        <table class="border-2" v-if="data !== null">
            <tr class="border-2  p-2">
                ...
                <td class="border-2  p-2 font-semibold">Насия</td>
            </tr>
            <tr class="border-2  p-2" v-for="(store, index) in data" :key="index">
                ...
                <td class="border-2  p-2">{{ store.debt }} c.</td>
            </tr>
...
<script>
    ...
    export default {
        ...
        async mounted() {
            check(this.$router)
            await axios.get(`${process.env.VUE_APP_API_URL}counts/topDebtors`)
                .then(resp => {
                    this.data = resp.data
                })
        }
